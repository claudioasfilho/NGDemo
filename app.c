...
//      app_log_info("Started advertising\n");
    ...
    case sl_bt_evt_gatt_server_characteristic_status_id:
      ...
      if (gattdb_App_Read == evt->data.evt_gatt_server_characteristic_status.characteristic) {
        ...
        if (sl_bt_gatt_server_client_config == (sl_bt_gatt_server_characteristic_status_flag_t)evt->data.evt_gatt_server_characteristic_status.status_flags) {
          app_log("Central has subscribed to Notifications \n");
        }
//        else {
//          app_assert(false,
...
//        }
    ...
    case sl_bt_evt_gatt_server_attribute_value_id:
      ...
      if (gattdb_App_Write == evt->data.evt_gatt_server_attribute_value.attribute) {
        uint8_t data_recv;
        size_t data_recv_len;

        // Read characteristic value.
        sc = sl_bt_gatt_server_read_attribute_value(gattdb_App_Write,
                                                    0,
                                                    sizeof(data_recv),
                                                    &data_recv_len,
                                                    &data_recv);
...
void start (sl_cli_command_arg_t *arguments)
{
  (void) arguments;
  sl_status_t sc;
...
void send (sl_cli_command_arg_t *arguments)
{
  (void) arguments;
...
//      rx);
  ...
  sc = sl_bt_gatt_server_notify_all(gattdb_App_Read,
                                    sizeof(data_send),
                                    &data_send);
  if (sc == SL_STATUS_OK) {
